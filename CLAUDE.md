...
```bash
sft rx -p <port> -k                              # receive
sft tx -ip4 <IPv4 addr> -p <port> -k <path>      # send over IPv4
sft tx -ip6 <IPv6 addr> -p <port> -k <path>      # send over IPv6
```

`-k` is **mandatory** on both sides. It is a standalone flag (no inline value); the passphrase is prompted from the controlling terminal with echo disabled. If stdin is not a TTY (e.g. `echo pw | sft ...`), one line is read plainly from stdin instead, which is what makes the tool scriptable. Both sides must enter the same passphrase or the receiver aborts on the first encrypted packet.
